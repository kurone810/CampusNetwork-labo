...
try {
    Write-LabLog -Message "=== VyOS VM の起動を開始します ===" -Level Info
    ...
    Get-VM | Where-Object { $_.Name -in $script:AllVyOSVMs } | Format-Table Name, State, Uptime -AutoSize
}
catch {
    Write-LabLog -Message "VyOS VM 起動中にエラーが発生しました: $_" -Level Error
    exit 1
}
